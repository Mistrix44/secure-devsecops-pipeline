...
function ContributionsHandler(db) {
    ...
    this.handleContributionsUpdate = (req, res, next) => {
        ...
        // Validate and parse contribution percentages as numeric input.
        // Reject any input containing non-digit characters.
        const preTax = /^\d+$/.test(req.body.preTax) ? parseInt(req.body.preTax, 10) : NaN;
        const afterTax = /^\d+$/.test(req.body.afterTax) ? parseInt(req.body.afterTax, 10) : NaN;
        const roth = /^\d+$/.test(req.body.roth) ? parseInt(req.body.roth, 10) : NaN;
